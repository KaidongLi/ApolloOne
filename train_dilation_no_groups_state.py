...
#def main(epoch):
	...
	#save_checkpoint({'epoch': parser.start-epoch, 'state_dict': retinanet.state_dict()})
	...
	for epoch_num in range(parser.start_epoch, parser.epochs):
		...
		torch.save(retinanet, '{}_retinanet_dilation_no_groups_{}.pt'.format(parser.dataset, epoch_num))

	retinanet.eval()

	torch.save(retinanet, 'model_final_dilation_no_groups.pt'.format(epoch_num))
